...
class MeroLaganiAPI:
    ...
    async def _get_html_content(self, url):
        html_response = await self.client.get(url)
    ...
    async def get_scrip_details(self, scrip_id):
        scrip_details_url = f"{self.end_point_scrip_details}{scrip_id}"

        page_content = await self._get_html_content(scrip_details_url)
    ...
    async def get_multiple_scrips_details(self, scrips):
        return await asyncio.gather(*list(map(self.get_scrip_details, scrips)))
